...
| 確認項目 | 内容 |
| --- | --- |
| Python環境 | 対話モードまたはスクリプトで例外を発生させて確認できる状態にしておく。 |
| 前提知識 | 関数、if文、ファイル操作の基本を確認しておく。 |
| 確認観点 | どの例外を捕捉したいのかを具体的に考える。 |

## 混乱しやすい点
| 混乱しやすい点 | 確認する観点 |
| --- | --- |
| 広すぎるexcept | `except Exception`だけに頼ると、原因を見失いやすい。必要な例外を絞る。 |
| BaseExceptionの扱い | SystemExitやKeyboardInterruptも含むため、通常の業務コードで安易に捕捉しない。 |
| 握りつぶし | exceptで何もせず通過すると、障害原因が分からなくなる。ログや再送出を検討する。 |
...
## 実務とのつながり
- 堅牢な処理<br>
    ファイル読み込み、外部API、ユーザー入力では例外処理が品質に直結する。
- 障害調査<br>
    例外クラスとスタックトレースを読むことで、原因箇所を特定しやすくなる。

## 要約
- 例外処理は、エラー発生時の流れを制御する仕組み。
- exceptでは、捕捉したい例外クラスをできるだけ具体的に指定する。
- finallyは後片付け、elseは例外が起きなかった場合の処理に使える。
