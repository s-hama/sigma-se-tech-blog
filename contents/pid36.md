...
| 確認項目 | 内容 |
| --- | --- |
| Python環境 | 対話モードでbin関数と演算結果を確認できる状態にしておく。 |
| 前提知識 | 2進数、10進数、ビットの基本を確認しておく。 |
| 確認観点 | 10進数の結果だけでなく、2進数表現も見ながら読む。 |

## 混乱しやすい点
| 混乱しやすい点 | 確認する観点 |
| --- | --- |
| 論理演算子との違い | `and`や`or`は真偽値の演算、`&`や`|`はビット単位の演算。 |
| 反転の結果 | Pythonの整数は固定ビット幅ではないため、`~x`は`-(x + 1)`として考えると分かりやすい。 |
| シフトの意味 | 左シフトは2倍方向、右シフトは2で割る方向の変化として確認する。 |
...
## 実務とのつながり
- フラグ管理<br>
    複数のON/OFF状態を1つの整数にまとめる処理で使われる。
- 低レイヤ理解<br>
    ネットワーク、ファイル形式、暗号処理ではビット単位の考え方が必要になる。

## 要約
- ビット演算子は、整数をビット列として扱う演算子。
- `|`、`^`、`&`はビットごとの比較、`<<`と`>>`はビットの移動を行う。
- 論理演算子とビット演算子は用途が異なる。
