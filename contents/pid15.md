## タイトル
Python - ニューラルネットワーク：1/14 単純パーセプトロンの仕組みと実装
...
## この記事で理解できること
- 入力、重み、しきい値、バイアスの役割。
- 単純パーセプトロンで論理回路を表す考え方。
- AND、NAND、ORをPython関数として実装する方法。
- 単純パーセプトロンの限界。

## 作業前に確認すること
| 確認項目 | 内容 |
| --- | --- |
| Python | 関数定義、if文、NumPy配列の基本を確認しておく。 |
| 数学 | 掛け算、足し算、不等号の読み方を押さえておく。 |
| 位置づけ | ニューラルネットワークの入口として、まず1つのニューロンの動作を理解する。 |

## 混乱しやすい点
| 混乱しやすい点 | 確認する観点 |
| --- | --- |
| しきい値とバイアス | しきい値を左辺に移したものがバイアスとして扱われる、と見ると整理しやすい。 |
| 学習との違い | ここでは重みを手で決めており、データから自動調整しているわけではない。 |
| XORが表せない理由 | 直線で分けられない問題は単純パーセプトロンだけでは扱えない。 |
...
パーセプトロンには、**入力層**と**出力層**のみの2層からなる**単純パーセプトロン**と、多層からなる**多層パーセプトロン**がある。ここでは、**単純パーセプトロン**の動作を実装サンプルで確認する。
...
### 単純パーセプトロンの実装サンプル
上記\\(（D）\\)を**ANDゲートで表現した**実装サンプルを説明する。

もちろん、**単純パーセプトロンの動作原理**の解説と同様に\\(（D）\\)についても NANDゲート、ORゲートは、パラメータ\\(w_{1}\\)（重み1）,\\(w_{2}\\)（重み2）, \\(b\\)（バイアス）を変えることで表現できる。
...
互いの配列要素が同じNumPy配列の乗算は、**要素同士が乗算される**ため、要素が\\(n\\)個であっても、上記実装サンプル（＊）の一行で実行できる。

重み（\\(w_{1}\\), \\(w_{2}\\)）の程度にもよるが、基準となるニューロン発火のしやすさ（出力が1となる度合）は、**バイアス**（\\(b\\)）が要となる。

## 実務とのつながり
- 分類モデルの原型<br>
    入力を重み付きで評価して出力を決める考え方は、後のニューラルネットワークにもつながる。
- モデルの説明力<br>
    単純な論理回路で確認すると、重みやバイアスが何を調整しているか理解しやすい。

## 要約
- 単純パーセプトロンは、入力と重みの合計をしきい値で判定するモデル。
- AND、NAND、ORは重みとバイアスを変えることで表現できる。
- XORのような線形分離できない問題には、多層構造が必要になる。
