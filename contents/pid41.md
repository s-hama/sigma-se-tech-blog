...
| 確認項目 | 内容 |
| --- | --- |
| 前提記事 | pid39とpid40の環境準備を済ませておく。 |
| API情報 | OpenWeatherMapのAPIキーとTelegram Botトークンを用意する。 |
| 実行環境 | Botを起動し続けられるPython実行環境を用意する。 |

## 注意したい点
| 注意したい点 | 確認する観点 |
| --- | --- |
| 状態が増えすぎる | すべてを状態として表すと複雑になりやすい。必要な状態を絞る。 |
| 入力の揺れ | ユーザーの表現は一定ではないため、形態素解析や条件判定で吸収する必要がある。 |
| 外部APIエラー | 天気APIが失敗した場合の応答も考えておく。 |
...
## 実務とのつながり
- 業務フローBot<br>
    問い合わせ、予約、申請など、決まった手順がある処理では状態遷移ベースが実装しやすい。
- 保守性<br>
    状態と遷移を整理しておくと、対話の分岐追加や不具合調査がしやすくなる。

## まとめ
- 状態遷移ベースでは、現在の状態と入力に応じて次の処理を決める。
- 天気案内Botでは、必要な情報を集めてから天気APIを呼び出す。
- 状態が複雑になりすぎないよう、対話設計を先に整理することが重要。
