...
[Python - ニューラルネットワーク： 損失関数と数値微分（勾配）の実装サンプル > 微分のおさらい](https://sigma-se.com/detail/24/#:~:text=%E3%82%8C%E3%81%A6%E3%81%84%E3%82%8B%E3%80%82-,%E5%BE%AE%E5%88%86%E3%81%AE%E3%81%8A%E3%81%95%E3%82%89%E3%81%84,-%E5%BE%AE%E5%88%86%E3%81%A8%E3%81%AF)
...
| 比較する項目 | 整理するポイント |
| --- | --- |
| 偏微分と通常の微分 | 他の変数を固定し、1つの変数だけを変化させる。 |
| 勾配の向き | 勾配は損失が増える方向を示すため、学習では逆方向へ進む。 |
| 破壊的変更 | 数値微分では配列の値を書き換えるため、元に戻す処理を忘れない。 |

## まとめ
- 偏微分は、複数変数のうち1つに注目して変化を調べる方法。
- 勾配は、各変数の偏微分をまとめたベクトル。
- 学習では、勾配と逆方向へパラメータを更新して損失を下げる。
