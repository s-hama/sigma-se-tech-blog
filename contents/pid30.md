...
| 確認項目 | 内容 |
| --- | --- |
| Python環境 | 対話モードでtypeや演算結果を確認できる状態にしておく。 |
| 前提知識 | 四則演算、比較演算、真偽値の基本を確認しておく。 |
| 確認観点 | 値だけでなく、演算後の型も確認する。 |
...
| 比較する項目 | 整理するポイント |
| --- | --- |
| boolはintの派生型 | Trueは1、Falseは0のように扱えるが、意味としては真偽値として読む。 |
| floatの誤差 | 10進数で自然に見える値でも、2進数では正確に表せないことがある。 |
| complexの表記 | Pythonでは虚数単位にjを使う。 |

## 実務とのつながり
- 条件分岐<br>
    boolの評価ルールはif文やwhile文の挙動に直結する。
- 計算処理<br>
    floatの誤差を理解しておくと、金額計算や比較処理でのバグを避けやすい。

## まとめ
- bool、int、float、complexはPythonの基本的な数値系データ型。
- 演算によって結果の型が変わることがある。
- floatの丸め誤差やcomplexの表記は、実装時に注意が必要。
