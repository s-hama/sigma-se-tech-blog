## タイトル
Python - タスク指向型対話 : 5/5 フレームベースのSVM（sklearn）モデル学習実装サンプル
...
下記の要領で推定結果`da`を返す。<br>
  ...
  - （＊10）単語配列（words）を空白区切りに置換え `tokens_str`に格納する。
  - （＊11）上記で読込んだ`vectorizer`を `TfidfVectorizer`を用いて素性ベクトルに変換する。
  - （＊12）（＊11）の`X`と上記で読み込んだ**svc**の`predict`を用いて推定結果を取得する。
  - （＊13）上記で読込んだ`label_encoder`をもとに数値列からラベル名に戻して返す。
