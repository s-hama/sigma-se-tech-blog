...
| 確認項目 | 内容 |
| --- | --- |
| 勾配 | 偏微分と勾配ベクトルの意味を確認しておく。 |
| NumPy | 配列演算と関数呼び出しの基本を確認しておく。 |
| 数学 | 関数の最小値を探すイメージを持っておく。 |

## 混乱しやすい点
| 混乱しやすい点 | 確認する観点 |
| --- | --- |
| 学習率の大きさ | 大きすぎると発散し、小さすぎると収束に時間がかかる。 |
| 局所的な最小値 | 関数の形によっては、全体の最小値ではない場所に向かうことがある。 |
| 勾配の符号 | 損失を下げるため、勾配そのものではなく逆方向へ動かす。 |
...
### 勾配降下法のPython実装サンプル
[Python - ニューラルネットワーク： 偏微分と勾配の実装サンプル > 勾配のPython実装サンプル](https://sigma-se.com/detail/25/#:~:text=%E3%81%AB%E9%81%8E%E3%81%8E%E3%81%AA%E3%81%84%E3%80%82-,%E5%8B%BE%E9%85%8D%E3%81%AEPython%E5%AE%9F%E8%A3%85%E3%82%B5%E3%83%B3%E3%83%97%E3%83%AB,-%E4%B8%8A%E8%A8%98%E3%81%A7%E3%80%81) で解説した**勾配関数**（num_gradient）を使用する。
...
## 実務とのつながり
- モデル学習の中心処理<br>
    実務で使うフレームワークも、内部では勾配を使ってパラメータ更新を行う。
- ハイパーパラメータ調整<br>
    学習率は学習の安定性に大きく影響するため、調整対象として重要。

## 要約
- 勾配降下法は、勾配の逆方向へ少しずつ進んで関数の値を小さくする方法。
- 学習率は一回の更新量を決める。
- ニューラルネットワークの重み更新を理解するための基本になる。
