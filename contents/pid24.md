...
| 確認項目 | 内容 |
| --- | --- |
| 数学 | 関数、傾き、微小な変化の考え方を確認しておく。 |
| Python | 関数を引数として渡す書き方を確認しておく。 |
| 損失関数 | 損失を小さくする方向を探すという目的を理解しておく。 |
...
但し、この概念を理解する上で注意しなければならないのが [Python - ニューラルネットワーク： 活性化関数の実装サンプルまとめ（ステップ、シグモイド、ReLU、恒等関数、ソフトマックス関数）> ステップ関数](https://sigma-se.com/detail/18/#:~:text=%E3%81%A8%E5%AE%9F%E8%A3%85%E3%82%B5%E3%83%B3%E3%83%97%E3%83%AB-,%E3%82%B9%E3%83%86%E3%83%83%E3%83%97%E9%96%A2%E6%95%B0,-%E5%89%8D%E3%81%AE%E8%A8%98%E4%BA%8B) のように、**\\(x = 0\\) 以外の値で微分結果が \\(0\\)（勾配が\\(0\\)　）となるような損失関数**では、そもそも正解に近づけようがないため、微分する意味がない。
...
| 比較する項目 | 整理するポイント |
| --- | --- |
| 解析的微分との違い | 数式で厳密に求めるのではなく、微小な差分で近似する。 |
| hの大きさ | 大きすぎると粗く、小さすぎると丸め誤差の影響を受けやすい。 |
| 中心差分 | 前方差分より中心差分の方が誤差を抑えやすい。 |

## 実務とのつながり
- 勾配確認<br>
    実装した勾配が正しいかを確認するため、数値微分はデバッグ用途でも使える。
- 学習処理の理解<br>
    自動微分ライブラリを使う前に、数値微分を知ると重み更新の意味を理解しやすい。

## まとめ
- 数値微分は、微小な差分から関数の傾きを近似する方法。
- 損失関数の傾きは、学習で重みをどちらに動かすかを決める手がかりになる。
- hの設定や中心差分の考え方が重要。
