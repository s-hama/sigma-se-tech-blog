...
- [MNISTとは](#mnistとは)<br>
  MNISTの意味と基本的な考え方を整理。
- [MNISTのデータ仕様](#mnistのデータ仕様)<br>
  MNISTのデータ仕様の意味と要点を具体例から整理。
- [MNISTのダウンロード](#mnistのダウンロード)<br>
  MNISTのダウンロードの手順と確認ポイントを整理。
- [画像データ表示確認](#画像データ表示確認)<br>
  画像データ表示について、確認する項目と結果の見方を整理。
...
- load_mnist関数の概要<br>
引数 normalize は、入力画像を**0.0 ～ 1.0 に正規化するかどうか**Bool値で設定。<br>
Falseの場合、入力画像のピクセルは 0 ～ 255 となる。<br>
引数 flatten は、入力画像を**1次元にするかどうか**Bool 値で設定。<br>
Falseの場合、入力画像は1 * 28 * 28 の3次元配列として格納され、Trueの場合、1次元配列(要素：784)として格納される。<br>
引数 one_hot_label は、ラベルを **one_hot表現で格納するか**Bool値で設定。<br>
...
## まとめ
- MNISTは手書き数字の画像と正解ラベルから構成され、画像はピクセル値の配列として扱われる。
- shapeや画像表示を確認すると、配列の形と実際の画像を対応付けやすい。
