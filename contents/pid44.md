...
- 桁落ち<br>
    ...
    - 例 : 各平方根を有効数字 \\(7\\) 桁に丸めて、\\(\sqrt{999} - \sqrt{997}\\) を計算する。
...
- 和集合<br>
    ...
    集合記号：\\(A \cup B\\)<br>
    \\(x \in A \cup B\\) は、\\(x \in A\\) または \\(x \in B\\)（OR）に対応する。

- 積集合<br>
    両方に共通して含まれる要素の集合。<br>
    集合記号：\\(A \cap B\\)<br>
    \\(x \in A \cap B\\) は、\\(x \in A\\) かつ \\(x \in B\\)（AND）に対応する。

- 補集合<br>
    全体集合 \\(U\\) のうち、集合 \\(A\\) に含まれない要素の集合。<br>
    集合記号：\\(A^c = U \setminus A\\)<br>
    \\(x \in A^c\\) は、\\(x \in A\\) ではないこと（NOT）に対応する。
...
- 対称差集合<br>
    ...
    集合記号：\\(A \triangle B\\)<br>
    \\(x \in A \triangle B\\) は、\\(x\\) がどちらか一方だけに属すること（XOR）に対応する。
...
| 補数と単なる符号付きの数 | \\(r\\) の補数は、決められた基数と桁数の中で負の数を表すための表現。 |
| 丸め誤差と桁落ち | 丸め誤差は桁を削る誤差、桁落ちは近い値同士の引き算で有効桁が減る誤差。 |
| 情報落ちとアンダーフロー | 情報落ちは小さい値が演算に反映されないこと、アンダーフローは絶対値が正規化数の最小値を下回ること。 |
...
- 補数表現<br>
    固定ビット幅の \\(2\\) の補数表現や、オーバーフローの挙動を理解する手がかりになる。
