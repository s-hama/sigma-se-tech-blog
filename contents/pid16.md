...
## この記事で扱うこと
- 単純パーセプトロンがXORを表現できない理由。
- 複数のパーセプトロンを組み合わせる考え方。
- 中間層を持つことで表現力が増える理由。
- XORをPythonで実装する流れ。
...
| 確認項目 | 内容 |
| --- | --- |
| 前回の理解 | AND、NAND、ORのパーセプトロン実装を確認しておく。 |
| Python | 関数を組み合わせて別の関数を作る流れを理解しておく。 |
| 図形的な理解 | 点を直線で分けるイメージを持っておくとXORの限界が分かりやすい。 |
...
| 比較する項目 | 整理するポイント |
| --- | --- |
| 多層化の意味 | 単に関数を増やすことではなく、中間的な表現を作れるようになる点が重要。 |
| XORだけが目的ではない | XORは、多層化によって表現力が増えることを示す代表例。 |
| 重みの決め方 | ここでは手で決めた重みを使い、学習による自動調整は後続の記事で扱う。 |

## 実務とのつながり
- 複雑な分類への入口<br>
    実務の分類問題では、単純な直線だけでは分けられないデータが多く、多層化の考え方が必要になる。
- 深層学習への接続<br>
    多層パーセプトロンは、後のニューラルネットワークや深層学習を理解する土台になる。

## まとめ
- 単純パーセプトロンだけではXORを表現できない。
- 複数のパーセプトロンを組み合わせると、中間層を使って表現力を高められる。
- 多層パーセプトロンはニューラルネットワークの基本構造につながる。
